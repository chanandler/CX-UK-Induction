...
enum PinKeychain {
    ...
    /// Removes the stored PIN and clears any lockout state. Returns true if the PIN was removed
    /// successfully or if no PIN existed.
    @discardableResult
            ...
            Form {
                if mode == .verify {
                    Section("PIN Required") {
                        Text(promptText ?? "Enter your PIN to access \(actionName).")
